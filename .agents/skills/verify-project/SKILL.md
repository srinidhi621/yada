...
## Local testing package without enrollment

Paid signing is deferred. Run `python3 scripts/package-local.py` to test/build/package an ad-hoc Release DMG, and `python3 -m unittest discover -s scripts -p "test_*.py"` for tooling checks. The script never installs or changes grants. Everyday testing uses `/Applications/Yada.app` following docs/releases.md; DerivedData apps remain for development only. Verify real Control-Y start/stop and insertion after installation. Signature and DMG checks do not prove Gatekeeper acceptance or retained grants across ad-hoc upgrades.
